...
(function () {
  ...
  var ZH = {
    ...
    'pg.sub': '这是一个真实的 MatrixOne 实例，不是录像。你打开这个页面的那一刻，系统就给了你一份 customers 表的独立分支——从快照 fork 出来的，不是拷贝。随便改，别人看不到，你离开后它就消失。',
    'pg.booting': '正在为你创建分支…',
    'pg.guide': '四个动词，按顺序来',
    'pg.hint': '点一个步骤把语句载入，然后运行。也可以完全不理这些，自己写 SQL —— 这是你的分支。',
    'pg.label': '要执行的 SQL',
    'pg.run': '运行',
    'pg.reset': '重置我的分支',
    'pg.idle': '结果会显示在这里。',
    ...
    'pg.howp': '每个访客都拿到一份真实、可写、互相隔离的同一张表。换任何别的数据库，这要么意味着每人一个容器，要么大家挤在一个 schema 里互相踩。而在这里它只是一条语句 —— <code>DATA BRANCH CREATE TABLE</code> —— 代价是元数据而不是拷贝，所以一台实例能装下上千个。这个 playground 的架构本身，就是整个网站在讲的那个主张。',
    'pg.limits': '边界说清楚：一次一条语句、5 秒超时、最多返回 200 行、每个分支 80 条语句、闲置 20 分钟后回收。会伸到你分支之外的语句一律拒绝。这些限制在你自己机器上跑 MatrixOne 时都不存在。',
